...
class JsonImportSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // SAFETY GUARD: clearAllData() truncates offert_position / position_materials / clients.
        // Never do that on a database that already holds offers. Override with FORCE_SEED=true.
        if (DB::table('offerts')->exists()
            && ! filter_var(env('FORCE_SEED', false), FILTER_VALIDATE_BOOLEAN)) {
            $this->command?->warn('JsonImportSeeder skipped: offers already exist (live data protected). Set FORCE_SEED=true to override.');
            return;
        }
