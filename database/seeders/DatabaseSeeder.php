...
class DatabaseSeeder extends Seeder
{
    ...
    {
        // SAFETY GUARD: never run seeders against a database that already holds offers.
        // Several catalog seeders (JsonImportSeeder, the fallback below, RepairConnectionsSeeder)
        // TRUNCATE offer/position pivots and clients — on a live DB that destroys real data.
        // Fresh installs (no offers) still seed normally. Override only deliberately, after a
        // backup, with FORCE_SEED=true.
        if (DB::table('offerts')->exists()
            && ! filter_var(env('FORCE_SEED', false), FILTER_VALIDATE_BOOLEAN)) {
            $this->command?->warn('DatabaseSeeder skipped: offers already exist (live data protected). Set FORCE_SEED=true to override.');
            return;
        }
